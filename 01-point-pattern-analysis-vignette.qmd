...
calcMetric <- function(fun, pp_ls, celltype, continuous = FALSE){
  res_df <- lapply(zstack_list, function(x){
    if(continuous){
      pp_sub <- subset(pp_ls, select = celltype)
    }
    else{
...
res <- calcMetric('markcorr', pp[[1]], genes, continuous = TRUE)
```
...
res <- calcMetric('Lcross', pp_ls, c('OD Mature', 'Microglia'))

q <- ggplot(res, aes(r, iso, colour = stack)) + geom_line() + theme_light() + geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.25, show.legend = FALSE) + geom_line(aes(x=r,y=theo),linetype = "dashed", color = 'black')
```
