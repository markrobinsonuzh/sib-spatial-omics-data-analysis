...
calcMetric <- function(fun, pp_ls, celltype){
  res_df <- lapply(zstack_list, function(x){
    res <- lohboot(pp_ls[[x]][[celltype]], fun = fun)
    res$stack <- x
    return(res)
  }) %>% bind_rows()
  return(res_df)
...
pp_ls <- lapply(pp_ls, separateMarks)

res1 <- calcMetric('Kest', pp_ls, 'OD Mature')

p <- ggplot(res1, aes(r, iso, colour = stack)) + geom_line() + theme_light() + geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.25, show.legend = FALSE)

res2 <- calcMetric('Lest', pp_ls,  'OD Mature')

q <- ggplot(res2, aes(r, iso, colour = stack)) + geom_line() + theme_light() + geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.25, show.legend = FALSE)
